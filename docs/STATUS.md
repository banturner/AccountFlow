...
| 11 | Graph provider first-poll and delta-cursor bugs — `$filter`/`$orderby` 400 `ErrorInefficientFilter` on every first poll; dropped `@odata.nextLink` stalling the cursor on any round past `max_results`; delta `isRead` updates replaying pre-connection mail into the pipeline; a stale `$skiptoken` 400 that no reset could clear; a missing parked delta link pinning a mailbox in first-poll mode | VERIFY 2026-09-21 | `services/outlook.py` `fetch_new_messages` / `_horizon`; 13 new tests in `tests/test_outlook_provider.py`. Found by code review and fixed, but **this code has still never talked to Graph** — VERIFY, not DONE, until Test B step 9 (`scripts/graph_live_smoke.py`) runs against a live tenant |
| 12 | Sync cursor is advanced past messages the worker then drops at the plan cap, and past mail lost in other non-consuming paths. Affects **both** providers (`sync_state` and `gmail_history_id`) — those emails are never shown to the clinic and are unreachable once the cursor moves | OPEN — **data loss, found 2026-09-21** | `worker/tasks.py` cap break at `_process_tenant_inbox_async`; `services/outlook.py`, `services/gmail.py`. Needs a decision: fetch only the remaining cap headroom, or roll the cursor back when the poll is not fully consumed |
| 13 | `_horizon` is anchored to `integrations.created_at`, which never moves. Reconnecting a **different** mailbox reuses the row (unique on `tenant_id,provider`) without resetting `created_at` or `sync_state`, so the floor and the cursor both belong to the old mailbox | OPEN | `services/outlook.py` `_horizon`, `api/routes/auth.py` microsoft callback. Real fix is a forward-moving `sync_started_at` column set on every OAuth callback and cursor reset — migration `009`, consult `architect` first |
| 14 | `get_access_token` has no tests on either provider — the row lock, refresh-token rotation, and the no-refresh-token branch are exercised by nothing. A rotated Microsoft refresh token is `flush()`ed, not committed, so a later failure in the same poll rolls it back while Microsoft has already rotated | OPEN | `services/outlook.py`, `services/gmail.py`. CLAUDE.md requires tests for any change to token storage |
