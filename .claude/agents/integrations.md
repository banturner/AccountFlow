...
## Hard rules
1. Minimum scopes, no silent escalation. Google today is `gmail.modify` + `calendar`; the review proposed narrowing to `gmail.readonly` + `gmail.send` + `calendar.events`, but narrowing invalidates every consented token (re-consent required), so it is a founder decision — propose, do not ship unasked. Microsoft delegated set: `Mail.Read`, `Mail.Send`, `Calendars.ReadWrite`, `offline_access`, `User.Read`. Never add a scope without stating in your summary why the current set cannot do the job.
